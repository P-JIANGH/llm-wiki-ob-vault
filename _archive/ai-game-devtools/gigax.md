...
Gigax 专注于**游戏运行时 NPC 动作决策**，而非通用 Agent 框架。与 [[ai-town]] 的对话式 AI 角色不同，Gigax 通过预定义 `Skill`（如 attack、move、say）约束 NPC 的动作空间，输出是结构化的可执行动作而非纯文本。相较于 [[cradle]] 的 screenshot→键鼠动作范式，Gigax 走的是结构化 LLM 生成路线，更接近 [[generative-agents]] 的 Memory Stream 记忆架构但专注于动作输出。Outlines guided generation 是其区别于大多数游戏 NPC LLM 方案的独特技术选型。
