...
## Overview
AudioGPT 是首个通过 ChatGPT/LLM 统一接口来理解和生成语音、音乐、音效和数字人视频的 AI 系统。借鉴 Visual ChatGPT 的设计模式，将 LLM 作为控制器，通过 [[ai-game-devtools/langchain]] Agent 框架调度多个音频基础模型，实现自然语言驱动的多模态音频交互。
...
| 任务 | 基础模型 | 状态 |
|------|---------|------|
| Talking Head Synthesis | [[ai-game-devtools/geneface]] | WIP |
...
- 相比 [[ai-game-devtools/amphion]]（统一音频工具包）：AudioGPT 侧重 LLM 驱动的自然语言交互，而非 API 级工具调用
- 相比 [[ai-game-devtools/musicgen]]（Meta 音乐生成）：AudioGPT 覆盖语音/音效/数字人等多模态，不仅限于音乐
- 作为 Audio 类最早的 ChatGPT 集成项目之一，启发了后续 [[ai-game-devtools/audio-diffusion-pytorch]] 等专用工具
