...
CTAG 通过**进化优化合成器参数**来从文本提示生成声音。与 [[ai-game-devtools/audioldm-2]] 等扩散模型不同，CTAG 不训练神经网络直接输出波形，而是将声音合成视为黑盒优化问题：搜索能使 CLAP 嵌入与文本语义匹配的合成器参数配置。
...
- 相比 [[ai-game-devtools/audioldm-2]]（扩散模型）：CTAG 使用进化优化而非神经网络生成，不依赖训练数据分布，更具创造性
- 相比 [[ai-game-devtools/audiolcm]]（LCM 加速扩散）：CTAG 不训练任何模型，而是实时搜索合成器参数空间，每次生成都是独立的优化过程
