...
- 基于 [[cf-3dgs]] 同类的 Gaussian Splatting 渲染技术，但 DreamGaussian4D 专注 4D 动态生成而非 3D 重建
- 与 [[dream-catalyst]] 同属 3D 编辑/生成工具，但 DreamGaussian4D 专攻 4D 时序
- 与 [[anything-3d]] 均从单视图生成 3D，但 DreamGaussian4D 额外增加时间维度
- 静态后端可替换为 DreamGaussian（未在本 wiki 中建页），与 [[syncdreamer]] 同属同步生成范式
- 使用 [[city-dreamer]] 类似的分阶段生成思想（布局→生成）
