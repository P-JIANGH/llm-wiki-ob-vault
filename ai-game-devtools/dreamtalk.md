...
- 对比 [[ai-game-devtools/aniportrait]]：两者均为音频驱动头像，DreamTalk 用扩散模型生成3DMM参数再渲染，AniPortrait 直接通过 wav2vec2 → audio2mesh/audio2pose → 3DMM + AnimateDiff Motion Module
- 对比 [[ai-game-devtools/ditto-talkinghead]]：两者均为扩散方法，DreamTalk 使用 3DMM 参数空间扩散 + PIRender 渲染，Ditto 使用 motion-space 扩散 (LMDM) + HuBERT 音频编码 + LivePortrait 面部操控
- 对比 [[ai-game-devtools/chatdollkit]]：ChatdollKit 是 Unity 实时语音对话 SDK（LLM+STT/TTS+VRM），DreamTalk 是离线视频生成模型
