...
在所有指标上超越 FoleyGrafter、V-AURA、Frieren、[[mmaudio]]、ThinkSound：
...
相比 [[foley-crafter]]（基于 Auffusion + ControlNet 时间适配器的视频→音效），HunyuanVideo-Foley 采用更完整的端到端多模态扩散架构，同时处理视觉和文本输入，在同步精度和音质保真度上全面领先。与 [[audioldm-2]] 等纯文本→音频模型不同，它额外利用视频视觉信息实现音画同步。
