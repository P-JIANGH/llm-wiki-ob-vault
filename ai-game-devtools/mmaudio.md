...
## 与同类工具对比
- 相比 [[foley-crafter]]（视频→Foley 音效，ControlNet 适配器），MMAudio 采用联合训练范式，同时支持文本条件和视频条件
- 相比 [[audiox]]（统一多模态音频生成，DiT + MAF），MMAudio 侧重视频-音频同步对齐，采用 Flow Matching 而非扩散模型
- 相比 [[hunyuanvideo-foley]]（腾讯混元视频音效生成），MMAudio 为学术开源项目，支持更灵活的文本+视频联合条件输入
...
## 致谢依赖
[[make-an-audio-3]] (BigVGAN 预训练模型 + VAE 架构), NVIDIA BigVGAN, Synchformer, EDM2 (VAE 架构)
