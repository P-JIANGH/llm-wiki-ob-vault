...
与 [[ai-game-devtools/motionllm]]（IDEA/清华的视频+动作联合理解LLM）不同，MotionGPT 专注于运动生成而不仅是理解——它不仅能"读懂"运动，还能"写出"运动。同时支持生成和翻译双向任务。

与 [[ai-game-devtools/hallo]]（复旦音频驱动肖像动画，SD+AnimateDiff架构）和 [[ai-game-devtools/liveportrait]]（快手视频驱动肖像动画）不同，MotionGPT 处理的是全身 3D 运动（22关节坐标），而非面部/头部动画。但三者结合可形成完整角色动画管线：文本→全身运动→面部表情→最终渲染。
