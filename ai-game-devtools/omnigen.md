...
与 [[controlnet]] 和 [[instantid]] 等需要额外条件网络的方式不同，OmniGen 在单一模型中自动识别输入图像特征（物体、姿态、深度等），根据文本提示决定如何利用参考信息。这简化了工作流，但模型参数量较大，内存占用较高。

与 [[comfyui]] 的节点式管线相比，OmniGen 提供极简 API（几行代码即可完成多种任务），但灵活性不如可视化节点编排。
