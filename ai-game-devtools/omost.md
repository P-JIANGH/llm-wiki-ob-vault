...
LLM 驱动的图像组合 (Image Composing) 工具，由 [[ai-game-devtools/controlnet]] 作者 lllyasviel 开发。将 LLM 的编码能力转化为图像构图能力——LLM 编写 Python Canvas API 代码来描述图像的空间布局和内容，然后由扩散模型渲染器实际生成图像。
...
- 与 [[ai-game-devtools/controlnet]] 不同：ControlNet 用条件图引导，Omost 用 LLM 代码描述
- 与 [[ai-game-devtools/comfyui]] 互补：ComfyUI 手动编排管线，Omost 自动化 LLM→代码→渲染
- 与 [[ai-game-devtools/rpg-diffusionmaster]] 类似理念：LLM 参与扩散过程的规划阶段
