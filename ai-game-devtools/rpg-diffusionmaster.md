...
- **vs [[controlnet]]**：ControlNet 通过条件图（姿态/深度/边缘）控制空间布局，RPG 通过 MLLM 语义理解自动分配区域，两者可结合使用
- **vs [[comfyui]]**：ComfyUI 是图形化节点工作流编辑器，RPG 是具体的算法框架，可在 ComfyUI 中集成
- **vs [[flux]]**：Flux 是新一代扩散模型架构，RPG 是方法论，可适配到 Flux 等后端
