...
相比 [[ai-game-devtools/wav2lip]]（专注唇形同步），SadTalker 生成完整的头部运动（表情+姿态+眨眼），不仅限于嘴部区域。相比 [[ai-game-devtools/hallo]]（扩散模型音频驱动），SadTalker 基于 3DMM 管线，计算开销更低但表情自然度稍弱。[[ai-game-devtools/liveportrait]] 则是视频驱动（非音频），需要参考视频而非音频输入。
