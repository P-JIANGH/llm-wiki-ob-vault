...
**Depth map library and poser** — 为 [[controlnet]] 扩展（A1111 WebUI）提供深度图库和可视化编辑器。
...
- 多深度图元素组合生成复杂姿态图
- 作为 [[openpose-editor]] 的补充，提供更简单的深度图编辑能力
...
- 相比 [[depth-anything-v2]]：Depth Anything V2 **生成**深度图（从 RGB 图像），而 sd-webui-depth-lib **管理和组合**预置深度图
- 相比 [[openpose-editor]]：Openpose Editor 专注于人体骨骼姿态编辑，sd-webui-depth-lib 更通用，支持任意深度图的叠加组合
- 与 [[controlnet]] 互补：ControlNet 消费深度图作为条件输入，sd-webui-depth-lib 提供深度图的生产与管理能力
