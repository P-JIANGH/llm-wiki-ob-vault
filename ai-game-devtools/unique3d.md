...
Unique3D 采用四阶段管线：**多视图扩散生成 → 法线预测 → 微分网格重建 → 纹理投影**。输入单张正视图，输出正面/右/背/左四视角一致的带纹理3D网格。与 [[crm]] 和 [[hunyuan3d-2-0]] 相比，Unique3D 强调推理速度（30秒）和网格质量（法线引导优化）。
...
与 [[stable-fast-3d]]（TripoSR改进版，<0.5秒但质量较低）和 [[threestudio]]（统一框架但需SDS优化数十分钟）相比，Unique3D 在速度和质量之间取得平衡：30秒生成高质量网格，但需要手动下载权重且依赖较多。[[crm]] 同样实现单图→3D网格（10秒），但使用FlexiCubes而非微分优化。
