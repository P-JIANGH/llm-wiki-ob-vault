...
- 支持 2-4 名玩家本地/联机（继承自 [open-source-game/fish-folk-jumpy](#/open-source-game/fish-folk-jumpy) 框架）
- 内置关卡编辑器（通过 WASM 前端）
- 完整本地化支持（多语言 `locales/`）

## 与 Fish Folk Jumpy 的关系

Punchy 与 [open-source-game/fish-folk-jumpy](#/open-source-game/fish-folk-jumpy) 同属 **Fish Folk 组织**（Spicy Lobster 社区）：
