...
这些文件与 [open-source-game/commander-genius](#/open-source-game/commander-genius) 共享相同的 id Software 引擎基因，但各有不同的实现路径。
...
### VANILLA 兼容模式
`VANILLA=ON` 编译选项禁用 Omnispeak 独有功能，强制 bug 兼容原版 DOS 体验，类似于 [open-source-game/chocolate-doom](#/open-source-game/chocolate-doom) 的 Bug-Compatible 哲学。
...
| id Software 引擎复用 | 经典引擎代码库复用模式，[open-source-game/scummvm](#/open-source-game/scummvm) 的插件引擎架构也有类似思路 |
| 存档兼容性 | VANILLA 模式的 bug 兼容策略，可参考用于 AI 游戏的"代际兼容"设计 |

## 相关页面

- [open-source-game/commander-genius](#/open-source-game/commander-genius) — Commander Keen 1-6 另一开源实现
- [open-source-game/chocolate-doom](#/open-source-game/chocolate-doom) — Bug-Compatible 重现哲学
- [open-source-game/scummvm](#/open-source-game/scummvm) — 经典游戏引擎多插件架构
