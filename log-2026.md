...
## [2026-04-10] ingest | DeerFlow 模块深度学习（Sandbox/Subagent/Memory/Runtime）
- Updated: entities/deer-flow.md（大幅扩充：完整包结构+路径映射+三线程池+Memory队列+StreamBridge+Checkpointer，11.5KB）
- Created: concepts/deer-flow-sandbox.md（双向路径翻译+输出反向掩码+segment boundary正则+LocalSandbox实现+安全门控）
- Created: concepts/deer-flow-subagent.md（三线程池架构+SubagentStatus+SSE事件流+Registry覆盖+与ClawTeam对比）
- Created: concepts/deer-flow-memory.md（Fact结构+category分类+Debounce队列+注入机制+与nanobot Dream对比）
- Created: concepts/deer-flow-runtime.md（RunManager+StreamBridge解耦+Checkpointer三层+Store+序列化）
- Updated: index.md（新增4条目，total: 109→113）
- Note: 精读源码：sandbox/tools.py(1345行)/local_sandbox.py/security.py + factory.py + memory/{updater,queue,prompt,storage}.py + subagents/{executor,registry,config}.py + runtime/{runs/manager,stream_bridge/base,serialization}.py + checkpointer/provider.py + skills/manager.py


- Created: raw/articles/clawteam-readme-2026.md（README + 架构文档，5.6KB）
- Created: entities/clawteam.md（完整条目：adapters/transport/task/mcp/workspace/swarm协调协议/Roadmap）
- Created: concepts/agent-swarm.md（Agent蜂群协作模式：Leader+Worker+inbox+git worktree）
- Created: entities/openclaw.md（OpenClaw + nanobot关系）
- Created: entities/hkuds.md（HKUDS组织 + nanobot/ClawTeam/CLI-Anything项目矩阵）
- Updated: index.md（新增5条目，total: 100→105）
- Note: 通过 ~/ClawTeam 源码精读 adapters.py/models.py/tasks.py/store/base.py/mcp/tools/team.py + README 845行完成

- Created: raw/articles/nanobot-readme-2026.md（README + 核心架构文档，6KB）
- Updated: entities/nanobot.md（大幅扩充：Provider Registry、Dream两阶段记忆、Tool Registry、Channel Bus 等详细内容）
- Created: concepts/agent-loop.md（AI Agent 核心执行循环，loop.py 750行 + runner.py 723行详解）
- Created: concepts/provider-registry.md（插拔式 LLM Provider 架构，2步添加 Provider 设计模式）
- Created: concepts/channel-system.md（Message Bus 解耦多聊天平台，13渠道插件化接入）
- Created: concepts/memory-system.md（Dream 两阶段系统：Consolidator + Dream，GitStore 版本化）
- Updated: index.md（新增 Concepts 章节，total pages: 96→100）
- Note: 通过 ~/nanobot 源码目录精读 loop.py/runner.py/memory.py/registry.py 等核心文件，结合 MEMORY.md 文档完成
...
- 修复 `!![open-source-game/hurry-curry]` → `[[hurry-curry]]`
...
- Note: 纯 C 代码库 C99 标准确保最大兼容性；与 [[devilutionx|EvilutionX]] 清洁室逆向相比，AbyssEngine 选择完全重写路线；LibArchive 直接读取 MPQ 容器复用原版数据是快速启动项目的好方法；VideoManager 处理 FFmpeg 视频解码是 2D 游戏过场动画的标准方案
